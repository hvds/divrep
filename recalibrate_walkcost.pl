...
my $timeout = 8;   # NOT arbitrary - 20s here OOM-killed this session's run:
                    # run_benchmark() buffers a benchmark's ENTIRE stdout in
                    # memory before parsing (millions of lines/many hundreds
                    # of MB for the 'baseline' batch within 20s), so this
                    # needs to stay well short of whatever memory is
                    # actually available. Raise with care, and only after
                    # checking memory headroom - or better, convert
                    # run_benchmark()/parse_stats() to stream line-by-line
                    # instead of slurping, which would remove this limit
                    # entirely (not done here - out of scope for what this
                    # session needed).
GetOptions(
    'pcoul=s'   => \$pcoul,
    'qs=s'      => \$qs_binary,
...
if ($qs_binary) {
    ...
    for my $i (1, 2) {
        ...
        print STDERR "[qs] timing reference semiprime $i (" .
            length($n) . " digits)...\n";
        my $t0 = time;
        # STANDALONE_SIMPQS's main() reads via gmp_scanf from stdin
        # (prints an "Input number to factor" prompt first) - NOT via
        # argv. Piping is required; passing $n as an argv argument
        # makes the driver block forever waiting on stdin, which looks
        # indistinguishable from "just very slow" unless you know to
        # check for it - this session's v0.54 recalibration hit exactly
        # this trap first (hvds' own branch had patched the driver to
        # accept argv for convenience; the vanilla v0.54 tag did not
        # pick that up).
        system("echo $n | $qs_binary > /dev/null 2>&1");
        $timed{$i} = time - $t0;
